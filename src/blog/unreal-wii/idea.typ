...
I've never done any programming for the Wii, so I'd like to get the toolchain setup and see some code running within #link("https://dolphin-emu.org")[Dolphin Emulator]. I came across #link("https://github.com/devkitPro")[devkitPro] which includes a toolchain for Wii/GC's PowerPC architecture, and a #link("https://github.com/devkitPro/wii-examples")[series of examples] including a hello triangle and audio! DevkitPro needs a posix style environment so it installed MSYS onto my Windows installation. In the future I'll likely be developing on Arch because having this weird fake linux thing on the horrible experience that is Windows is gonna be frustrating. But all I had to do was fire up MSYS and run make on a few examples and load them into Dolphin! All worked out the box!

#image("../../assets/unreal-wii/wii_hello_triangle.png")
...
- A simple rendering backend for the Wii
  - There's an #link("https://github.com/dborth/sdl-wii")[SDL] and #link("https://github.com/devkitPro/pacman-packages/blob/master/wii/SDL2/PKGBUILD")[SDL2] implementation for the Wii! Hopefully this is all we'll need for a primitive renderer
