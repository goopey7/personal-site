...
We could try creating a page file backed #link("https://learn.microsoft.com/en-us/windows-hardware/drivers/kernel/section-objects-and-views")[section object] and see what happens. All virtual memory in Windows is backed by a file just in case we run out of physical memory. Unfortunately, this is a part of the Windows driver development kit, and I don't want to make a driver, I just want a userspace program. Fortunately there's another established API known as #link("https://learn.microsoft.com/en-us/windows/win32/memory/sharing-files-and-memory")[File Mapping]. If we pass an `INVALID_HANDLE_VALUE` for `hFile` in `CreateFileMapping`, the mapping object will access memory backed by the system paging file. Here's a quick and dirty example of this API in action. I have to run this in administrator mode in order to make use of the global namespace. There might be a way around that, but running servers in admin mode is not a deal breaker either.
...
So we know it's possible to share large blocks of memory between processes on Windows. What about Linux?
...
Can we get Unreal servers to operate in a shared memory architecture? What would the developer experience be like? How would a gamedev using my artefact articulate what should be shared and what should not?
